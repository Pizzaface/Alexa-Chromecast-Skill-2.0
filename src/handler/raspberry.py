...
# Connect String
db = pymysql.connect("RASP_PI_DNS", user="MYSQL_USER",
                     passwd="MYSQL_PASS", db="DB_NAME")

# Sets up The Chromecast For Use (Will Run any time there is an issue
...
def sendVideo(url):
    # Tries to wait until the Chromecast is ready (Simply Tests for Chromecast
    # Connection)
    try:
        cast.wait()
    except:
        ...
        # Uses Youtube-Dl to get the Stream URL for the Video to send to
        # Chromecast
...
def volumeSet(Volnum):
    try:
        ...
        # Tries to wait until the Chromecast is ready (Simply Tests for
...
def dbConnect():
    ...
    # Loops through the row to see what command it was to send it to the right
    # function
